...
public:
	//skin mesh related
	static const int maxBoneBufferSize = 300; //ボーンの最大数
	...
	struct cbObject
	{
		...
		//skin meshオブジェクトかどうか
		int IsSkinMeshObj = 0;  // one int (4-byte)CPU & GPU 
		...
		//float			_blank2 = 0.0f; (add an int isSkin so cancel this to counterpart)

		//water shader1　constant buffer
		int WaterEnable = 0; //有効化
		Math::Matrix mR; //for rotation 90degree rotation
		Math::Vector2 WaterUVOffset;  		//twist the normal to create water's distribution　補正値
		float dummy[5] = { 0,0 ,0,0,0};
	...
	// 定数バッファ(ボーン単位更新:skinMesh include)
	struct cbBone
	{
	...
	//water shader 2 prepare setter
	//for sending normal map 
	void SetWaterNormalTexture(KdTexture& _normalTex) //send texture to GPU, slot 7 = camera , texture = 9 slot,  send to slot 9 this time
	{
		KdDirect3D::Instance().WorkDevContext()->PSSetShaderResources(9, 1, _normalTex.WorkSRViewAddress());
	}
	...
	void DrawModel(KdModelWork& rModel, const Math::Matrix& mWorld = Math::Matrix::Identity,
		const Math::Color& colRate = kWhiteColor, const Math::Vector3& emissive = { 1,1,1 });
...
private:
	...
	KdConstantBuffer<cbBone>		m_cb3_Bone;				// skin Mesh ボーン毎に更新
