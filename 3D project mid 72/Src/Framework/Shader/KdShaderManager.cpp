...
void KdShaderManager::Init()
{
	...
	//黒崎先生3　init constant buffer
	m_cb10_Effect.Create();
	m_cb10_Effect.Write();
	KdDirect3D::Instance().WorkDevContext()->VSSetConstantBuffers(10, 1, m_cb10_Effect.GetAddress()); //where to send the data
...
{
	//m_cb9_Light.Work().ConeLights.enable = _enable;

	m_cb9_Light.Write();//this command is to send data from cpu to gpu
}

void KdShaderManager::WriteCBConeLights(const std::list<ConeLight>& coneLights)
{
	cbLight& light = m_cb9_Light.Work();

	//light.ConeLight_Num = std::min(static_cast<int>(coneLights.size()), cbLight::MaxConeLightNum);

	light.ConeLight_Num = coneLights.size();

	int index = 0;
	for (const auto& cone : coneLights)
	{
		if (index >= cbLight::MaxConeLightNum)
			break;

		light.ConeLights[index] = cone;
		++index;
	}
...
void KdShaderManager::WriteCBColor(const std::list<ColorLight>& colorLights)
{
	cdEffect& effect = m_cb10_Effect.Work();

	// Limit the number of ColorLights to MAX_COLOR_LIGHTS (17)
	effect.ColorLight_Num = std::min(static_cast<int>(colorLights.size()), cdEffect::MaxColorLightNum);

	int index = 0;
	for (const auto& colorLight : colorLights)
	{
		if (index >= cdEffect::MaxColorLightNum)
			break;

		effect.ColorLights[index] = colorLight;
		++index;
	}
...
void KdShaderManager::WriteCBSingleLightEnable(bool _enable)
{
	m_cb9_Light.Work().SingleLight.enable = _enable;
	m_cb9_Light.Write();
}

void KdShaderManager::WriteCBSingleLight(const Math::Vector3& _pos, const Math::Vector3& _dir, const float _range, const float _angle, const Math::Vector3& _color)
{
	m_cb9_Light.Work().SingleLight.pos = _pos;
	m_cb9_Light.Work().SingleLight.dir = _dir;
	m_cb9_Light.Work().SingleLight.range = _range;
	m_cb9_Light.Work().SingleLight.angle = _angle;
	m_cb9_Light.Work().SingleLight.color = _color;

	m_cb9_Light.Write();
}

void KdShaderManager::WriteCBSingleLightEnable2(bool _enable)
{
	m_cb9_Light.Work().SingleLight2.enable = _enable;
	m_cb9_Light.Write();
}

void KdShaderManager::WriteCBSingleLight2(const Math::Vector3& _pos, const Math::Vector3& _dir, const float _range, const float _angle, const Math::Vector3& _color)
{
	m_cb9_Light.Work().SingleLight2.pos = _pos;
	m_cb9_Light.Work().SingleLight2.dir = _dir;
	m_cb9_Light.Work().SingleLight2.range = _range;
	m_cb9_Light.Work().SingleLight2.angle = _angle;
	m_cb9_Light.Work().SingleLight2.color = _color;
...
void KdShaderManager::CallImgui()
{

	//imgu begin
	//ImGui::Begin("ShaderManager");

	//if (m_cb9_Light.Get().ConeLight_Num > 0)
	//{
	//	ConeLight& coneLight = m_cb9_Light.Work().ConeLights[0];

	//	// Display a checkbox to enable or disable the cone light
	//	ImGui::Text("Enable Cone Light : %d", &coneLight.enable);

	//	
	//	ImGui::SliderFloat("Position X", &coneLight.pos.x, -100.0f, 100.0f);
	//	ImGui::SliderFloat("Position Y", &coneLight.pos.y, -100.0f, 100.0f);
	//	ImGui::SliderFloat("Position Z", &coneLight.pos.z, -100.0f, 100.0f);

	//	// Display direction control (as a vector3)
	//	ImGui::InputFloat3("Direction", &coneLight.dir.x);

	//	// Display controls for range and angle
	//	ImGui::SliderFloat("Range", &coneLight.range, 0 , 35);
	//	ImGui::SliderFloat("Angle", &coneLight.angle, 0.0f, 35);

	//	// Display color control (as a vector3)
	//	ImGui::ColorEdit3("Color", &coneLight.color.x);

	//	// After editing the values, you might want to write them back to the constant buffer
	//	//m_cb9_Light.Write();
	//}
	...
	// Access the list of cone lights from m_ambientController
	auto& coneLights = m_ambientController.m_coneLights;

	// Loop through the cone lights and display their attributes
	int lightIndex = 0;
	for (auto& coneLight : coneLights)
	{
		ImGui::PushID(lightIndex);  // Ensure each light has a unique ID in ImGui

		ImGui::Text("Cone Light %d", lightIndex + 1);

		// Display enable checkbox
		//ImGui::Checkbox("Enable", &coneLight.enable);

		// Display position controls
		ImGui::InputFloat3("Position", &coneLight.pos.x);

		// Display direction controls
		ImGui::InputFloat3("Direction", &coneLight.dir.x);

		// Display range and angle controls
		ImGui::InputFloat("Range", &coneLight.range);
		ImGui::SliderFloat("Angle", &coneLight.angle, 0.0f, 180.0f);
