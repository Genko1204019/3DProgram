...
public:
	...
	void DrawString(float _x, float _y, const wchar_t _text[], const Math::Vector4& _color, float scale = 1.0f, int _fontStyle = 1, float rotation = 0.0f, DirectX::FXMVECTOR origin = { 0,0,0,0 })
	{
		ID3D11BlendState* oldBlendState = nullptr;
		float oldFactor[4];
		UINT oldMask = 0;
		KdDirect3D::Instance().GetDevContextss()->OMGetBlendState(&oldBlendState, oldFactor, &oldMask);
		spriteBatch->Begin(DirectX::SpriteSortMode_Deferred, oldBlendState);

		if (_fontStyle == 1) {
			spriteFont->DrawString(spriteBatch, _text, Math::Vector2(_x + 640, -_y + 360), _color, rotation, origin, scale);
		}
		else if (_fontStyle == 2) {
			spriteFont2->DrawString(spriteBatch, _text, Math::Vector2(_x + 640, -_y + 360), _color, rotation, origin, scale);
		}

		spriteBatch->End();

		KdDirect3D::Instance().GetDevContextss()->OMSetBlendState(oldBlendState, oldFactor, oldMask);
		KdSafeRelease(oldBlendState);
	}

	void DrawString(float _x, float _y, const char _text[], const Math::Vector4& _color, float scale = 1.0f, int _fontStyle = 1, float rotation = 0.0f, DirectX::FXMVECTOR origin = { 0,0,0,0 })
	{
		...
		if (_fontStyle == 1) {
			//spriteFont->DrawString(spriteBatch, _wtext, Math::Vector2(_x + 635, -_y + 360), { 0,0,0,_color.w }, rotation, origin, scale * 1.14); //for outline
			spriteFont->DrawString(spriteBatch, _wtext, Math::Vector2(_x + 640, -_y + 360), _color, rotation, origin, scale);

		}
		else if (_fontStyle == 2) {
			spriteFont2->DrawString(spriteBatch, _wtext, Math::Vector2(_x + 640, -_y + 360), _color, rotation, origin, scale);
		}
...
private:
	...
	DirectX::SpriteFont* spriteFont2;
};
