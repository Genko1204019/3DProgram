...
void KdEffekseerManager::StopEffectById(int _uniqueId)
{
	// Iterate through the list of currently playing effects
	for (auto itr = m_nowEffectPlayList.begin(); itr != m_nowEffectPlayList.end(); ++itr)
	{
		auto effObj = *itr;
		if (effObj && effObj->GetUniqueId() == _uniqueId)
		{
			// Stop the effect using its handle
			m_efkManager->StopEffect(effObj->GetHandle());

			// Remove the effect from the list
			m_nowEffectPlayList.erase(itr);
			return;  // Exit after stopping the effect
		}
	}
}
...
void KdEffekseerManager::SetRotationById(int _effectId, Math::Vector3& _axis, float _angle)
{
	Effekseer::Vector3D efkAxis = GetEfkVec3D(_axis);
	m_efkManager->SetRotation(GetEffectHandleById(_effectId), efkAxis, _angle);
}
...
void KdEffekseerManager::SetWorldMatrixById(int _effectId, Math::Matrix& _mWorld)
{
	Effekseer::Matrix43 mEfk{};
	...
	for (int i = 0; i < 4; ++i)
	{
		for (int j = 0; j < 3; ++j)
		{
			mEfk.Value[i][j] = _mWorld.m[i][j];
		}
	}

	m_efkManager->SetMatrix(GetEffectHandleById(_effectId), mEfk);
...
void KdEffekseerManager::SetScaleById(int _effectId, Math::Vector3& _scale)
{
	float scaleX = _scale.x;
	float scaleY = _scale.y;
	float scaleZ = _scale.z;

	m_efkManager->SetScale(GetEffectHandleById(_effectId), scaleX, scaleY, scaleZ);
}
...
void KdEffekseerManager::SetSpeedById(int _effectId, float _speed)
{
	m_efkManager->SetSpeed(GetEffectHandleById(_effectId), _speed);
}
...
void KdEffekseerManager::SetPauseById(int _effectId, bool _isPause)
{
	m_efkManager->SetPaused(GetEffectHandleById(_effectId), _isPause);
}
...
{
	PlayEfkInfo updatedInfo = info;
	if (updatedInfo.uniqueId == -1)
	{
		updatedInfo.uniqueId = m_nextUniqueId++;
	}
...
Effekseer::Handle KdEffekseerManager::GetEffectHandleByName(const std::string& effectName)
{
	{
		for (const auto& effectObject : m_nowEffectPlayList) {
			if (effectObject && effectObject->GetPlayEfkInfo().FileName == effectName) {
				return effectObject->GetHandle();
			}
		}
		return -1; // Return an invalid handle if not found
	}
}

Effekseer::Handle KdEffekseerManager::GetEffectHandleById(int uniqueId)
{
	for (const auto& effectObject : m_nowEffectPlayList)
	{
		if (effectObject && effectObject->GetUniqueId() == uniqueId)
		{
			return effectObject->GetHandle();
		}
	}
...
void KdEffekseerManager::CallImgui()
{

	ImGui::Begin("Effect List");

	//show m_nextUniqueId
	ImGui::Text("Next Unique ID: %d", m_nextUniqueId);

	if (ImGui::CollapsingHeader("Effects", ImGuiTreeNodeFlags_DefaultOpen)) {
		// List all effects
		for (const auto& effect : m_nowEffectPlayList) {
			if (effect) {
				ImGui::PushID(effect.get());  

				ImGui::Text("Effect Name: %s", effect->GetPlayEfkInfo().FileName.c_str());
				ImGui::Text("Position: (%.2f, %.2f, %.2f)", effect->GetPos().x, effect->GetPos().y, effect->GetPos().z);
				ImGui::Text("Size: (%.2f, %.2f, %.2f)", effect->GetSize().x, effect->GetSize().y, effect->GetSize().z);
				ImGui::Text("Speed: %.2f", effect->GetSpeed());
				ImGui::Text("Is Playing: %s", effect->IsPlaying() ? "Yes" : "No");
				ImGui::Text("Is Looping: %s", effect->IsLoop() ? "Yes" : "No");
				ImGui::Text("Unique ID: %d", effect->GetUniqueId());

				//ImGui::SliderFloat3("Set Position", &effect->GetPos().x, -100.0f, 100.0f);
				//ImGui::SliderFloat3("Set Size", &effect->GetSize().x, 0.1f, 10.0f);
				//ImGui::SliderFloat("Set Speed", &effect->GetSpeed(), 0.0f, 5.0f);
