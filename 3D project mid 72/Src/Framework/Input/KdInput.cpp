...
void KdInputManager::CallImgui()
{
	//show all information of KdInputManager
	ImGui::Begin("KdInputManager");

	// Show the number of input devices
	ImGui::Text("Number of Input Devices: %d", m_pInputDevices.size());

	// Iterate through all input devices
	for (const auto& devicePair : m_pInputDevices)
	{
		const std::string& deviceName = devicePair.first;
		const std::unique_ptr<KdInputCollector>& collector = devicePair.second;

		if (ImGui::TreeNode(deviceName.c_str()))
		{
			// Show ActiveState of the collector
			const char* stateStr = "Unknown";
			switch (collector->GetActiveState())
			{
			case KdInputCollector::ActiveState::Disable:
				stateStr = "Disabled";
				break;
			case KdInputCollector::ActiveState::Monitoring:
				stateStr = "Monitoring";
				break;
			case KdInputCollector::ActiveState::Enable:
				stateStr = "Enabled";
				break;
			}
			ImGui::Text("Active State: %s", stateStr);

			// Show buttons information
			if (ImGui::TreeNode("Buttons"))
			{
				for (const auto& buttonPair : collector->m_spButtons)
				{
					const std::string& buttonName = buttonPair.first;
					const std::shared_ptr<KdInputButtonBase>& button = buttonPair.second;

					if (ImGui::TreeNode(buttonName.c_str()))
					{
						short state = button->GetState();
						ImGui::Text("State: %d (Press=%d, Hold=%d, Release=%d)", state, state & KdInputButtonBase::Press, state & KdInputButtonBase::Hold, state & KdInputButtonBase::Release);

						// Display key codes associated with this button
						std::vector<int> codes;
						button->GetCode(codes);
			...
			// Show axes information
			if (ImGui::TreeNode("Axes"))
			{
				for (const auto& axisPair : collector->m_spAxes)
				{
					const std::string& axisName = axisPair.first;
					const std::shared_ptr<KdInputAxisBase>& axis = axisPair.second;

					if (ImGui::TreeNode(axisName.c_str()))
					{
						Math::Vector2 state = axis->GetState();
						ImGui::Text("State: (X: %.2f, Y: %.2f)", state.x, state.y);
						ImGui::Text("Value Rate: %.2f", axis->m_valueRate);
						ImGui::Text("Limit Value: %.2f", axis->m_limitValue);
						ImGui::TreePop();
					}
				}
				ImGui::TreePop();
