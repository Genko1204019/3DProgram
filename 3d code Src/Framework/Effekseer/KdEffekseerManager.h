...
public:
	...
	struct PlayEfkInfo
	{
		...
		//add  a unique id for each effect
		int uniqueId = -1;
	};
	...
	void StopEffectById(int _uniqueId);
	...
	void SetScaleById(int _effectId, Math::Vector3& _scale);

	void SetSpeed(const int handle, const float speed = 1.0f);

	void SetSpeedById(int _effectId, float _speed);

	void SetPause(const int handle, const bool isPause);

	void SetPauseById(int _effectId, bool _isPause);
	...
	Effekseer::Handle GetEffectHandleByName(const std::string& effectName);
	Effekseer::Handle GetEffectHandleById(int uniqueId);
...
public:
	...
	void SetUniqueId(int id) { m_info.uniqueId = id; }
	int GetUniqueId() const { return m_info.uniqueId; }
	...
	//Get unique id
	//const int GetUniqueId() const { return m_info.uniqueId; }
